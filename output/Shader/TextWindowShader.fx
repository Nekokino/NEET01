...
VTXTEX_OUTPUT VS_TextWindow(VTXTEX_INPUT _In)
{
...
cbuffer SecondUV : register(b4)
{
	float4 SecUv;
}

PX_OUTPUT PX_TextWindow(VTXTEX_OUTPUT _In)
{
	float Standard = Start.y + Size.y * 0.5f;

	float Minus = Standard - SecUv;
	float Plus = Standard + SecUv;
	float Now = Start.y + (Size.y * _In.Uv.y);
